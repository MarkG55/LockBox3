...
begin
  ...
  if System.RTLVersion >= 19.00 then
    Check((Ver >= 27) and (Ver <= 41), 'These unit tests were ONLY made for revisions 27 through to 41 of D-Unit.')
  else
    Check(Ver = 36, 'For D7 and D2007 these unit tests were ONLY made for revision 36 of D-Unit.');
  // Nota bene: revision 41 is not compatible with Delphi 7.

  Check((System.RTLVersion = 15.00) or (System.RTLVersion = 17.00) or (System.RTLVersion = 18.00) or (System.RTLVersion = 21.00)
    or (System.RTLVersion = 23.00) or (System.RTLVersion = 27.00),
  ...
  if SizeOf(TrueNativeInt) <> SizeOf(pointer) then
    ShowMessageFmt('Integer   = %d bytes'#13#10 + 'NativeInt = %d bytes'#13#10 + 'TrueNativeInt = %d bytes'#13#10 +
      'Pointer = %d bytes'#13#10 + 'Compiler = %.2f bytes', [SizeOf(integer), SizeOf(NativeInt), SizeOf(TrueNativeInt),
      SizeOf(pointer), CompilerVersion]);
  Check((SizeOf(TrueNativeInt) = SizeOf(pointer)) and (SizeOf(TrueNativeUInt) = SizeOf(pointer)),
  ...
  Announcement := Announcement + Format('The SVN revision of the ' +
    ...
    'Check the TPLB3 project options for the run-time package library version.', [TPLB3Runtime_SVN_Revision]);

  Announcement := Announcement + #13#10;
  Announcement := Announcement + Format('You are testing with compiler %s', [DelphiVersion_DisplayName]);
