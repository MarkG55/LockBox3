...
type
  ...
  protected
    function Chain_EncryptBlock(Key: TSymetricKey; InitializationVector: TMemoryStream; const Cipher: IBlockCodec): TBlockChainLink;

    function Chain_DecryptBlock(Key: TSymetricKey; InitializationVector: TMemoryStream; const Cipher: IBlockCodec): TBlockChainLink;
  ...
  protected
    constructor Create(Key1: TSymetricKey; // Will be referenced, not cloned.
...
function TCTR.Chain_DecryptBlock(Key: TSymetricKey; InitializationVector: TMemoryStream; const Cipher: IBlockCodec): TBlockChainLink;
begin
  result := TCTRLink.Create(Key, InitializationVector, Cipher)
end;

function TCTR.Chain_EncryptBlock(Key: TSymetricKey; InitializationVector: TMemoryStream; const Cipher: IBlockCodec): TBlockChainLink;
begin
  result := TCTRLink.Create(Key, InitializationVector, Cipher)
end;
...
begin
  result := 'http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf' + '|section 6.5'
end;
...
constructor TCTRLink.Create(Key1: TSymetricKey; IV1: TMemoryStream; Cipher1: IBlockCodec);
begin
