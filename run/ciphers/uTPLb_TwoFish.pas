...
{$ENDIF}
{$IF compilerversion >= 21} [DesignDescription('From Wikipedia: QUOTE:'#13#10 + 'Twofish is a symmetric key block cipher with a block size of 128 bits and '#13#10 + 'key sizes up to 256 bits. It was one of the five finalists of the Advanced '#13#10 + 'Encryption Standard contest, but was not selected for standardisation. '#13#10 + 'Twofish is related to the earlier block cipher Blowfish.'#13#10 + 'END QUOTE')]
{$ENDIF}

  TTwoFish = class(TInterfacedObject, IBlockCipher, ICryptoGraphicAlgorithm, IControlObject
{$IF compilerversion < 21}
    , IVariableSeedSize
{$ENDIF}
    )
...
procedure TTwofishBlockCodec.Encrypt_Block(Plaintext, Ciphertext: TMemoryStream);
var
  InData, OutData: T128;
begin
...
procedure TTwofishBlockCodec.Decrypt_Block(Plaintext, Ciphertext: TMemoryStream);
var
  InData, OutData: T128;
begin
