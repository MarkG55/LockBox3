...
{$IF compilerversion >= 21} [DesignDescription('XXTEA is implemented here with the following options set so:'#13#10 + '(*) The encoding of longwords is little-endien.'#13#10 + '(*) Empty plaintext message maps to empty ciphertext message.'#13#10 + '(*) For plaintext messages less than or equal to 199 bytes:'#13#10 + '(**) The end of the plaintext is padded out PKCS7 style (RFC 3852) to 4-byte alignment;'#13#10 + '(**) 8 bytes of salt is appended to the plaintext;'#13#10 + '(**) The resultant extended plaintext is encrypted in XXTEA long block mode -'#13#10 + '      That is to say, the block size is set to be the message size.'#13#10 + '(*) For plaintext messages more than or equal to 200 bytes:'#13#10 + '(**) 12 random bytes are appended to the plaintext;'#13#10 + '(**) The block size is fixed to 212 bytes;'#13#10 + '(**) Normal block-cipher encryption applies according to your selected'#13#10 + '      chaining mode. As usual, this includes 64 bits of automatic salting,'#13#10 + '      and smart block quantisation (Ciphertext stealing or key-streaming,'#13#10 + '      as appropriate to the chaining mode).')]
{$ENDIF}

  TXXTEA_LargeBlock = class(TInterfacedObject, IStreamCipher, ICryptoGraphicAlgorithm, IControlObject)
  private
...
procedure XXTEA_Encrypt( // Corrected Block TEA encryption primitive.
  const Key: TTEA_Key; const PlainText: TLongWordDynArray; // At least 2
...
procedure XXTEA_Decrypt( // Corrected Block TEA encryption primitive.
  const Key: TTEA_Key; const CipherText: TLongWordDynArray; // At least 2
...
type
  ...
  TXXTEA_LargeBlock_LE_Encryptor = class(TInterfacedObject, IStreamEncryptor, IBlockCipherSelector)
  public
  ...
  TXXTEA_LargeBlock_LE_Decryptor = class(TInterfacedObject, IStreamDecryptor, IBlockCipherSelector)
  public
  ...
  TXXTEA_Block = class(TInterfacedObject, IBlockCipher, ICryptoGraphicAlgorithm)
  private
    FBlockSize: integer;
    function DisplayName: string;
    function ProgId: string;
...
const
  DELTA = $9E3779B9; // For Little-endien implementations only.

function Mx(const z, y, sum: longword; const k: TTEA_Key; const p, e: longword): longword;
...
function TXXTEA_LargeBlock.Parameterize(const Params: IInterface): IStreamCipher;
var
...
function TXXTEA_LargeBlock.Start_Encrypt(Key: TSymetricKey; CipherText: TStream): IStreamEncryptor;
begin
  result := TXXTEA_LargeBlock_LE_Encryptor.Create(self, Key as TXXTEA_LE_Key, CipherText)
end;

function TXXTEA_LargeBlock.Start_Decrypt(Key: TSymetricKey; PlainText: TStream): IStreamDecryptor;
begin
  result := TXXTEA_LargeBlock_LE_Decryptor.Create(self, Key as TXXTEA_LE_Key, PlainText)
end;
...
constructor TXXTEA_LargeBlock_LE_Encryptor.Create(Owner1: TXXTEA_LargeBlock; Key: TXXTEA_LE_Key; CipherText: TStream);
begin
  FOwner := Owner1;
  FKey := Key;
...
constructor TXXTEA_LargeBlock_LE_Decryptor.Create(Owner1: TXXTEA_LargeBlock; Key: TXXTEA_LE_Key; PlainText: TStream);
begin
  FOwner := Owner1;
  FKey := Key;
    ...
    begin
      ...
      if FBufLen >= DecryptionBufferSize then
        begin // Large message case, switch to adapter.
          ...
          FOutputBuffer := TAbbrieviatingStream.Create(FPlainText, LargeMessageSaltTailLength);
...
constructor TAbbrieviatingStream.Create(OutputStream1: TStream; ClipAmount1: integer);
begin
...
function TAbbrieviatingStream.Seek(const Offset: int64; Origin: TSeekOrigin): int64;
begin
  result := 0 // Not used
end;
    ...
    begin
      ...
      if Amnt > 0 then
        begin
          ...
          if Succeeded <> Amnt then
            break // Fail. Don't go further.
        end
    end
end;
