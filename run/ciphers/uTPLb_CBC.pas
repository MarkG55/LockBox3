...
function TCBC.Chain_DecryptBlock(Key: TSymetricKey; InitializationVector: TMemoryStream; const Cipher: IBlockCodec): TBlockChainLink;
begin
  result := TCBCLink.BaseCreate(Key, InitializationVector, Cipher);
  TCBCLink(result).FNextCV := TMemoryStream.Create
end;

function TCBC.Chain_EncryptBlock(Key: TSymetricKey; InitializationVector: TMemoryStream; const Cipher: IBlockCodec): TBlockChainLink;
begin
  result := TCBCLink.BaseCreate(Key, InitializationVector, Cipher)
end;
...
begin
  result := 'http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf' + '|section 6.2'
end;
