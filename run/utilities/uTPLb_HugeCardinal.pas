...
{$ENDIF}
    ...
    procedure DividePrimitive(Divisor: THugeCardinal; var Quotient, Remainder: THugeCardinal; RequiresQuotient: boolean);
...
{$ENDIF}
    ...
    constructor CreateFromStreamIn(MaxBits1: integer; ByteOrder: TByteOrder; Stream: TStream; const Pool1: IMemoryStreamPool);
    ...
    function PowerMod_WithChineseRemainderAlgorithm(Exponent, // d
    ...
    // Streaming.
    procedure StreamOut(ByteOrder: TByteOrder; Stream: TStream; SizeToOutput: integer = -1);
...
type
  TProfileSection = (proc_Add, proc_Increment, proc_Subtract, proc_AddMod, proc_MulSmall, func_Multiply, proc_MultiplyMod,
    proc_SquareMod, proc_MulPower2, func_Modulo, proc_Divide, func_PowerMod, proc_SmallExponent_PowerMod);
...
var
  ActiveProfileSections: TProfileSectionSet = [proc_MultiplyMod, proc_SquareMod, proc_MulPower2, func_Modulo];
...
class function THugeCardinal.ComputedNeededSize(RequestedBits: integer): integer;
begin
...
constructor THugeCardinal.CreateFromStreamIn(MaxBits1: integer; ByteOrder: TByteOrder; Stream: TStream;
...
constructor THugeCardinal.CreateRandom(Bits1, MaxBits1: integer; ExactBitLength: boolean; const Pool1: IMemoryStreamPool);
begin
...
procedure THugeCardinal.RandomBits(BitsOfRandom: integer; ExactBitLength: boolean);
var
  R: TStream;
  Bits, j: integer;
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_Add);
{$ENDIF}
  if Addend.isZero then
    exit;
  if isZero then
    begin
      Assign(Addend);
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_AddMod);
{$ENDIF}
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_Increment);
{$ENDIF}
  if Addend = 0 then
    exit;
...
begin
  if Modulus <= 1 then
    ...
    begin
      ...
      else if isSmall and (int64rec(Modulus).Hi = 0) and (int64rec(Low64).Hi = 0) then
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(func_Modulo);
{$ENDIF}
  Dummy := nil;
...
procedure THugeCardinal.Divide(Divisor: THugeCardinal; var Quotient, Remainder: THugeCardinal);
begin
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_Divide);
{$ENDIF}
  DividePrimitive(Divisor, Quotient, Remainder, True)
end;

procedure THugeCardinal.DividePrimitive(Divisor: THugeCardinal; var Quotient, Remainder: THugeCardinal; RequiresQuotient: boolean);
var
...
begin
  ...
  try
    ...
    while ((Compare in [rGreaterThan, rEqualTo]) or (ShiftAmnt > 0)) and (RemainderBits > 0) do
      begin
        LeftShift := DiffBits;
        ...
        if Compare in [rGreaterThan, rEqualTo] then
          begin
            ...
            if RequiresQuotient then
              begin
                Assert((QuotientByte^ and Mask) = 0, AS_HugeCardinal_DivideLogicFail);
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_MulPower2);
{$ENDIF}
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_MulSmall);
{$ENDIF}
  if isZero then
    exit;
  Done := False;
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(func_Multiply);
{$ENDIF}
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_MultiplyMod);
{$ENDIF}
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_SquareMod);
{$ENDIF}
  if isZero then
    exit;
...
procedure THugeCardinal.SmallExponent_PowerMod(Exponent: uint64; Modulus: THugeCardinal);
var
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(func_PowerMod);
{$ENDIF}
  Factor := nil;
  try
    doAbort := False;
...
function THugeCardinal.PowerMod_WithChineseRemainderAlgorithm(Exponent, // d
  Modulus: THugeCardinal; // n  := p * q
  FactorP, // p
  FactorQ, // q
  ExponentModFactorP, // dp := d mod p-1
  ExponentModFactorQ, // dq := d mod q-1
  InverseQ: THugeCardinal; // qinv := q ** -1 mod p
  OnProgress: TProgress): boolean; // result := self ** d mod n
var
...
begin
  if (not assigned(FactorP)) or (not assigned(FactorQ)) then
    ...
    begin
      ...
      x2 := Clone;
      try
        Result := PowerMod(ExponentModFactorP, FactorP, OnProgress) and x2.PowerMod(ExponentModFactorQ, FactorQ, OnProgress);
        if Result then
          begin
...
{$IFDEF profiling}
  if doProfiling then
    EnterProfileSection(proc_Subtract);
{$ENDIF}
  ...
  for j := 0 to (Bytes div 8) - 1 do
    begin
      ...
      if D2 > 0 then
        Move(P2^, V2, Min(D2, 8));
      if (V2 = 0) and (not Borrow) and (Idx >= SubBytes) then
        exit;
...
procedure THugeCardinal.StreamOut(ByteOrder: TByteOrder; Stream: TStream; SizeToOutput: integer = -1);
// ^ Streams the low order SizeToOutput bytes out to the stream.
// If SizeToOutput is -1, then this is interpreted as (MaxBits + 7) div 8
var
  P8: PByte;
...
procedure THugeCardinal.AssignFromStreamIn(ByteOrder: TByteOrder; Stream: TStream);
var
  P8: PByte;
  PayloadByte: byte;
  j: integer;
