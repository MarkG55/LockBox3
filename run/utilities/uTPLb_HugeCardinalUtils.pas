...
function GeneratePrime(NumBits: integer; OnProgress: TProgress; OnPrimalityTest: TPrimalityTestNoticeProc; PassCount: integer;
  const Pool1: IMemoryStreamPool; var Prime: THugeCardinal; var NumbersTested: integer): boolean;
...
procedure Compute_RSA_Fundamentals_2Factors(RequiredBitLengthOfN: integer; Fixed_e: uint64; // Put as -1 to generate a random number.
  // A value of StandardExponent is recommended.
  var N, e, d, Totient: THugeCardinal; var p, q, dp, dq, qinv: THugeCardinal; // For use with CRT.
  OnProgress: TProgress; OnPrimalityTest: TPrimalityTestNoticeProc; GeneratePrimePassCount: integer; // 1 .. 20;
  const Pool1: IMemoryStreamPool; var NumbersTested: integer; var wasAborted: boolean);
    ...
    for j := 0 to SmallPrimeSetCardinality - 1 do
      begin
        ...
        if p >= EratosthenesSieveSize then
          raise Exception.CreateFmt(ES_EratosthenesSievSizeTooSmall, [EratosthenesSieveSize, SmallPrimeSetCardinality])
      end;
  finally
    Numbers.Free
  end
  ...
  try
    for j := 1 to SmallPrimeSetCardinality - 1 do
      begin
        ...
        if doSmallTest and (q >= pValue) then
          break;
...
function extended_gcd(a, b: THugeCardinal; var x, y: THugeCardinal; var xPve, yPve: boolean; var gcd: THugeCardinal): boolean;
// Solve:
// a*x + b*y = gcd(a,b)
...
begin
  NextX := THugeCardinal.CreateSimple(0);
  NextXpve := True;
  x := THugeCardinal.CreateSimple(1);
  xPve := True;
  NextY := THugeCardinal.CreateSimple(1);
  NextYpve := True;
  y := THugeCardinal.CreateSimple(0);
  yPve := True;
  a1 := a.Clone;
  b1 := b.Clone;
...
begin
  ...
  if not TestPassed then
    begin
      result.Free;
      TestPassed := extended_gcd(Prime, Modulus, result, Dummy1, resultPve, DummyBool2, Dummy2);
  ...
  try
    for j := Low(SmallPrimes) + 1 to High(SmallPrimes) do
      begin
        ...
        if (SmallPrime = 0) or (Rem.CompareSmall(SmallPrime) in [rEqualTo, rLessThan]) then
          break;
...
function Validate_CRT_Numbers(N, e, d, p, q, dp, dq, qinv: THugeCardinal): boolean;
var
...
procedure Compute_RSA_Fundamentals_2Factors(RequiredBitLengthOfN: integer; Fixed_e: uint64;
  // Put as -1 to generate a random number.
  // A value of StandardExponent is recommended.
  var N, e, d, Totient: THugeCardinal; var p, q, dp, dq, qinv: THugeCardinal; // For use with CRT.
  OnProgress: TProgress; OnPrimalityTest: TPrimalityTestNoticeProc; GeneratePrimePassCount: integer; // 1 .. 20; 20 is good.
  const Pool1: IMemoryStreamPool; var NumbersTested: integer; var wasAborted: boolean);
...
begin
  ...
  if Fixed_e <> -1 then
    Assert((RequiredBitLengthOfN - BitCount_64(Fixed_e)) >= 16, AS_HugeCardinal_N_tooSmall_for_n);
  p := nil;
  q := nil;
  dp := nil;
  dq := nil;
  ...
  try
    try
      repeat
        // Step 3
        FreeAndNil(p);
        wasAborted := not GeneratePrime(p_bits, OnProgress, OnPrimalityTest, GeneratePrimePassCount, Pool1, p, NumbersTested);
        if wasAborted then
          break;
        ...
        // Step 5
        FreeAndNil(q);
        wasAborted := not GeneratePrime(q_bits, OnProgress, OnPrimalityTest, GeneratePrimePassCount, Pool1, q, NumbersTested);
        if wasAborted then
          break;
        ...
        if Validate_CRT_Numbers(N, e, d, p, q, dp, dq, qinv) then
          break;
      until wasAborted finally FreeAndNil(pMinus1);
      FreeAndNil(qMinus1)
    end;
  except
    begin
