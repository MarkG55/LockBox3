...
begin
  ...
  if CanManageSize(Size) then
    for j := 0 to AnticipatedPeakUsage - 1 do
      begin
        if j >= FCacheSegmentInUse.Size then
          break;
        if FCacheSegmentInUse[j] then
          continue;
        Idx := j;
        break
      end;
...
procedure TAllocationRecordListObj.GetUsage(Size: integer; var Current, Peak: integer);
