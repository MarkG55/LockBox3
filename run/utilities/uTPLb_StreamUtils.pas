...
procedure CustomBase64_to_stream(const Base64: TBytes; Destin: TStream; const InverseTransform: TInverseBaseTransform);
...
var
  L: integer;
begin
  if not assigned(Destructo) then
    exit;
  L := Destructo.Size;
  if L <= 0 then
    exit;
...
function TDesalinationWriteStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
