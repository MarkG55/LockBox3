...
type
  ...
  TOnHashProgress = function(Sender: TObject; CountBytesProcessed: int64): boolean of object;
...
begin
  ...
  while (Count > 0) and (not FisUserAborted) do
    begin
      ...
      if InSize < FInputBufferLen then
        break;
      FHasher.Update(FInputBuffer);
      FInputBuffer.Position := 0;
      InSize := 0;
      if assigned(FOnProgress) and (not FOnProgress(FSender, FCount)) then
        begin
          FisUserAborted := True;
          End_Hash;
          break
