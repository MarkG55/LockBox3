...
procedure TSignatory.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
begin
  ...
  if assigned(FCryptoKeys) then
    FCryptoKeys.Burn;
  FreeAndNil(FSigningKeys);
  FreeAndNil(FCryptoKeys)
end;
...
begin
  ...
  Engine.Sign(Document, Signature, FSigningKeys.FPrivatePart, self, FCodec.OnProgress, wasAborted);
...
begin
  ...
  Engine := FCodec.Asymetric_Engine;
  FCodec.EstimatedWorkLoad := Document.Size;
  Ok := Engine.VerifySignature(Document, Signature, FSigningKeys.FPublicPart, self, FCodec.OnProgress, wasAborted);
  FCodec.isUserAborted := wasAborted;
  if wasAborted then
    result := vUserAbort
  else if Ok then
...
procedure TSignatory.StoreKeysToStream(Store: TStream; Parts: TKeyStoragePartSet);
begin
  if assigned(FCryptoKeys) then
...
procedure TSignatory.LoadKeysFromStream(Store: TStream; Parts: TKeyStoragePartSet);
var
  Engine: IASymetric_Engine;
begin
  Engine := FCodec.Asymetric_Engine;
...
begin
  result := assigned(FSigningKeys) and assigned(FCryptoKeys) and FCryptoKeys.Can_StoreToStream(Parts) and
