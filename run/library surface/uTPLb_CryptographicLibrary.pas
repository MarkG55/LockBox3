...
type
  ...
  TCryptoLibStringRef = (cStreamId, sStreamName, cBlockId, cBlockName, cChainId, cChainName, cHashId, cHashName);

  TCryptographicLibrary = class;

  TOnGenerateKeyFunc = function(Lib: TCryptographicLibrary; Seed: TStream): TSymetricKey of object;

  TOnStart_EncryptFunc = function(Lib: TCryptographicLibrary; Key: TSymetricKey; CipherText: TStream): IStreamEncryptor of object;

  TOnStart_DecryptFunc = function(Lib: TCryptographicLibrary; Key: TSymetricKey; PlainText: TStream): IStreamDecryptor of object;
...
{$ENDIF}
  ...
  public
    ...
    function RegisterSimpleBlockTransform(Cls: TSimpleBlockCipherClass; const ProgId1: string; const DisplayName1: string;
    ...
    class function ComputeCipherDisplayName(const SCipher: IStreamCipher; const BCipher: IBlockCipher): string;
    function GetHashChoices: IInterfaceList; // of IHashDsc.

    class function ComputeHashDisplayName(const Hash: IHashDsc): string;

    function GetChainChoices: IInterfaceList; // of IChainMode.

    class function ComputeChainDisplayName(const Chain: IBlockChainingModel): string;
    ...
    property StreamCipherDisplayNames[const ProgIdx: string]: string read GetStreamCipherDisplayNames;
    ...
    property BlockCipherDisplayNames[const ProgIdx: string]: string read GetBlockCipherDisplayNames;
    ...
    property ChainModesDisplayNames[const ProgIdx: string]: string read GetChainModesDisplayNames;

    property Hashs_ByProgId: TStrings read GetHashs_ByProgId;
    property Hashs_ByDisplayName: TStrings read GetHashs_ByDisplayName;
    property HashDisplayNames[const ProgIdx: string]: string read GetHashDisplayNames;
  published
    property ParentLibrary: TCryptographicLibrary read FParentLibrary write SetParentLibrary;
  ...
  ICipherChoice = interface
    ...
    procedure GetChoiceParams(var CipherDisplayName: string; var isBlockCipher: boolean; var StreamCipherId: string;
...
type
  ...
  public
    constructor Create(Ref1: TCryptoLibStringRef; Lib1: TCryptographicLibrary);
    destructor Destroy; override;
...
class function TCryptographicLibrary.ComputeChainDisplayName(const Chain: IBlockChainingModel): string;
begin
  result := '';
...
class function TCryptographicLibrary.ComputeCipherDisplayName(const SCipher: IStreamCipher; const BCipher: IBlockCipher): string;
var
...
begin
  ...
  else if (afBlockAdapter in SCipher.Features) and assigned(BCipher) then
    begin
      isBlock := True;
      ...
      if (SizeParam > 0) and (Pos('%d', result) > 0) then
...
class function TCryptographicLibrary.ComputeHashDisplayName(const Hash: IHashDsc): string;
begin
  result := '';
...
function TCryptographicLibrary.BlockChainingModelIntfc(const ProgIdx: string): IBlockChainingModel;
var
  Idx, j: integer;
begin
  result := nil;
...
function TCryptographicLibrary.BlockCipherIntfc(const ProgIdx: string): IBlockCipher;
var
  Idx, j: integer;
begin
  result := nil;
...
procedure TCryptographicLibrary.DegisterWatcher(const Registrant: ICryptographicLibraryWatcher);
begin
  FWatchers.Remove(Registrant)
end;

procedure TCryptographicLibrary.DeregisterBlockChainingModel(const Registrant: IBlockChainingModel);
begin
  FChainModes.Remove(Registrant);
  ProgIdsChanged(MaxProgIdChangeCallFrames)
end;

procedure TCryptographicLibrary.DeregisterBlockCipher(const Registrant: IBlockCipher);
begin
...
procedure TCryptographicLibrary.DeregisterStreamCipher(const Registrant: IStreamCipher);
begin
  FStreamCiphers.Remove(Registrant);
  ProgIdsChanged(MaxProgIdChangeCallFrames)
end;

function TCryptographicLibrary.GetBlockCipherDisplayNames(const ProgIdx: string): string;
var
  Cipher: IBlockCipher;
begin
...
function TCryptographicLibrary.GetChainModesDisplayNames(const ProgIdx: string): string;
var
...
type
  TCipherChoice = class(TInterfacedObject, ICipherChoice, IBlockCipherSelector)
  private
    ...
    constructor Create(const BlockCipher1: IBlockCipher; const StreamCipher1: IStreamCipher);
  end;
...
var
  ...
  begin
    ...
    for j := 0 to Lib.FStreamCiphers.Count - 1 do
      if Supports(Lib.FStreamCiphers[j], IStreamCipher, StreamCipher1) and (StreamProgIds.IndexOf(StreamCipher1.ProgId) = -1) then
        begin
  ...
  begin
    for j := 0 to Lib.FBlockCiphers.Count - 1 do
      if Supports(Lib.FBlockCiphers[j], IBlockCipher, BlockCipher1) and (BlockProgIds.IndexOf(BlockCipher1.ProgId) = -1) then
        begin
  ...
  begin
    if assigned(Addend) and (DisplayNames.IndexOf(Addend.FDisplayName) = -1) and
...
var
  ...
  begin
    for j := 0 to Lib.FChainModes.Count - 1 do
      if Supports(Lib.FChainModes[j], IBlockChainingModel, Chain) and (ChainProgIds.IndexOf(Chain.ProgId) = -1) and
        (DisplayNames.IndexOf(Chain.DisplayName) = -1) and (not(afForRunTimeOnly in Chain.Features)) then
        begin
...
var
  ...
  begin
    for j := 0 to Lib.FHashes.Count - 1 do
      if Supports(Lib.FHashes[j], IHashDsc, Hash) and (HashProgIds.IndexOf(Hash.ProgId) = -1) and
        (DisplayNames.IndexOf(Hash.DisplayName) = -1) and (not(afForRunTimeOnly in Hash.Features)) then
        begin
...
function TCryptographicLibrary.GetHashDisplayNames(const ProgIdx: string): string;
var
  Hash: IHashDsc;
begin
...
function TCryptographicLibrary.GetStreamCipherDisplayNames(const ProgIdx: string): string;
var
...
procedure TCryptographicLibrary.RegisterBlockChainingModel(const Registrant: IBlockChainingModel);
var
  Idx: integer;
begin
...
procedure TCryptographicLibrary.RegisterBlockCipher(const Registrant: IBlockCipher);
var
  Idx: integer;
begin
...
function TCryptographicLibrary.RegisterSimpleBlockTransform(Cls: TSimpleBlockCipherClass; const ProgId1, DisplayName1: string;
  Features1: TAlgorithmicFeatureSet; BlockSizeInBytes1: integer): string;
var
...
procedure TCryptographicLibrary.RegisterStreamCipher(const Registrant: IStreamCipher);
var
  Idx: integer;
begin
...
procedure TCryptographicLibrary.RegisterWatcher(const Registrant: ICryptographicLibraryWatcher);
begin
  FWatchers.Add(Registrant)
end;

function TCryptographicLibrary.StreamCipherIntfc(const ProgIdx: string): IStreamCipher;
var
  Idx, j: integer;
begin
  result := nil;
  if FisDestroying then
    exit;
  if (FCustomStreamCipher.FProgId <> '') and (FCustomStreamCipher.FProgId = ProgIdx) then
    begin
...
begin
  if StackLimit <= 0 then
    exit; // Protect against stack overflow.
...
constructor TCryptLibStrings.Create(Ref1: TCryptoLibStringRef; Lib1: TCryptographicLibrary);
var
  ParentRef: ^TStrings;
begin
  FReference := Ref1;
  ...
  if assigned(FLib) then
    case FReference of
      cStreamId:
        ParentRef := @FLib.FStreamCiphers_ByProgId;
      sStreamName:
        ParentRef := @FLib.FStreamCiphers_ByDisplayName;
      cBlockId:
        ParentRef := @FLib.FBlockCiphers_ByProgId;
      cBlockName:
        ParentRef := @FLib.FBlockCiphers_ByDisplayName;
      cChainId:
        ParentRef := @FLib.FChainModes_ByProgId;
      cChainName:
        ParentRef := @FLib.FChainModes_ByDisplayName;
      cHashId:
        ParentRef := @FLib.FHashs_ByProgId;
      cHashName:
        ParentRef := @FLib.FHashs_ByDisplayName;
    end;
...
begin
  ...
  if assigned(FLib) then
    case FReference of
      cStreamId:
        result := (FLib.FStreamCiphers[Index] as IStreamCipher).ProgId;
      sStreamName:
        result := (FLib.FStreamCiphers[Index] as IStreamCipher).DisplayName;
      cBlockId:
        result := (FLib.FBlockCiphers[Index] as IBlockCipher).ProgId;
      cBlockName:
        result := (FLib.FBlockCiphers[Index] as IBlockCipher).DisplayName;
      cChainId:
        result := (FLib.FChainModes[Index] as IBlockChainingModel).ProgId;
      cChainName:
        result := (FLib.FChainModes[Index] as IBlockChainingModel).DisplayName;
      cHashId:
        result := (FLib.FHashes[Index] as IHashDsc).ProgId;
      cHashName:
        result := (FLib.FHashes[Index] as IHashDsc).DisplayName;
    end
end;
...
begin
  ...
  if assigned(FLib) then
    case FReference of
      cStreamId, sStreamName:
        result := FLib.FStreamCiphers.Count;
      cBlockId, cBlockName:
        result := FLib.FBlockCiphers.Count;
      cChainId, cChainName:
        result := FLib.FChainModes.Count;
      cHashId, cHashName:
        result := FLib.FHashes.Count;
    end
end;
...
constructor TCipherChoice.Create(const BlockCipher1: IBlockCipher; const StreamCipher1: IStreamCipher);
begin
  ...
  FDisplayName := TCryptographicLibrary.ComputeCipherDisplayName(FParamStreamCipher, FBlockCipher)
end;
...
procedure TCipherChoice.GetChoiceParams(var CipherDisplayName: string; var isBlockCipher: boolean;
  var StreamCipherId, BlockCipherId: string);
begin
...
type
  TCustomStreamCipherObj = class(TInterfacedObject, ICryptoGraphicAlgorithm, IStreamCipher)
  private
...
function TCustomStreamCipherObj.Parameterize(const Params: IInterface): IStreamCipher;
begin
  result := nil
end;
...
function TCustomStreamCipherObj.Start_Decrypt(Key: TSymetricKey; PlainText: TStream): IStreamDecryptor;
begin
...
function TCustomStreamCipherObj.Start_Encrypt(Key: TSymetricKey; CipherText: TStream): IStreamEncryptor;
begin
