...
type
  TSimpleCodec = class(TInterfacedPersistent, ICodec, IBlockCipherSelector, IBlockCipherSelectorEx2, IEventOrigin,
    ICodec_WithAsymetricSupport)
  private
    FMode: TCodecMode;
  ...
  public
    ...
    procedure Sign(Document, Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress;
      SigningKeys_PrivatePart: TObject; // Must be uTPLb_Asymetric.TAsymtricKeyPart
      var wasAborted: boolean);

    function VerifySignature(Document, Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress;
...
{$ENDIF}
  ...
  TCodec = class(TTPLb_BaseNonVisualComponent, ICryptographicLibraryWatcher, ICodec_TestAccess)
...
{$ENDIF}
  ...
  private
    ...
    procedure GenerateAsymetricKeyPairProgress_Event(Sender: TObject; CountPrimalityTests: integer; var doAbort: boolean);
    function GetAsymetricKeySizeInBits: cardinal;
    procedure SetAsymetricKeySizeInBits(Value: cardinal);
    function GetKey: TSymetricKey;
  ...
  published
    ...
    property AsymetricKeySizeInBits: cardinal read GetAsymetricKeySizeInBits write SetAsymetricKeySizeInBits;
...
begin
  ...
  FisUserAborted := assigned(FOnProgress) and (not FOnProgress(FSender, FXtextCount))
end;
...
begin
  ...
  result := assigned(FParameterizedStreamCipher) and (afAsymetric in FParameterizedStreamCipher.Features) and
...
function isVariableSeedSize(const Cipher: IStreamCipher; var MinSize, MaxSize: integer): boolean;
...
{$IF compilerversion >= 21}
  ...
  try
    ...
    for Meth in LType.GetDeclaredMethods do
      begin
        if not SameText(Meth.Name, 'SeedByteSize') then
          continue;
        for Attr in Meth.GetAttributes do
          begin
            if not(Attr is IntegerRange) then
              continue;
...
function ConstraintWithinRange(Instrinsic, LowerBound, UpperBound: integer): integer;
...
function CalculateOutputSeedSize(const Cipher: IStreamCipher; InputSeedSize: integer; var MinSize, MaxSize: integer): integer;
...
procedure SpreadHashIntoBuffer(Buffer: TMemoryStream; const PasswordHasher: IHash; MinSize, MaxSize, OutputSeedSize: integer);
...
begin
  ...
  try
    ...
    OutputSeedSize := CalculateOutputSeedSize(FParameterizedStreamCipher, InputSeedSize, MinSize, MaxSize);
      ...
      begin
        if InputSeedSize = OutputSeedSize then
          ...
          begin
            ...
            SpreadHashIntoBuffer(FBuffer, FPasswordHasher, MinSize, MaxSize, OutputSeedSize)
          end;
        FBuffer.Position := 0
      end;
...
begin
  if FAdvancedOptions2 = Value then
    exit;
  ...
  if Value <> [] then
    raise Exception.Create('Advanced options are still in development and not yet available'#13#10 +
      'They should be available in the next release'#13#10 + 'Please watch the shout-box on the LockBox home website for'#13#10 +
      'breaking news.');
...
begin
  ...
  if (not assigned(FStreamCipher)) or (afNotImplementedYet in FStreamCipher.Features) or

    ((afBlockAdapter in FStreamCipher.Features) and ((not assigned(FBlockCipher)) or (afNotImplementedYet in FBlockCipher.Features) or
    (not assigned(FChainMode)) or (afNotImplementedYet in FChainMode.Features))) then
...
begin
  ...
  useDesalination := ([afCompressor, afConverter, afDoesNotNeedSalt] * FParameterizedStreamCipher.Features) = [];
...
const
  FileStreamOpenMode: array [boolean] of word = (
    { False, Not exists } fmCreate, { True, exists } fmOpenReadWrite);

procedure TSimpleCodec.DecryptFile(const Plaintext_FileName, CipherText_FileName: string);
var
...
procedure TSimpleCodec.EncryptFile(const Plaintext_FileName, CipherText_FileName: string);
var
  Plaintext, CipherText: TStream;
  isPreExists: boolean;
begin
  ...
  try
    ...
    CipherText := TFileStream.Create(CipherText_FileName, FileStreamOpenMode[isPreExists]);
    try
      if isPreExists then
        CipherText.Size := 0;
...
begin
  ...
  if (FXtextCount = 0) and (L > 0) and (not FisSalting) and
    (([afCompressor, afConverter, afDoesNotNeedSalt] * FParameterizedStreamCipher.Features) = []) then
    begin
      FisSalting := True;
...
procedure TSimpleCodec.DecryptMemory(const CipherText { in }; CiphertextLen: integer);
var
  P: PByte;
  L, Amt: integer;
begin
...
begin
  if FAsymetricKeySizeInBits = Value then
    exit;
  if FMode in [cmEncrypting, cmDecrypting] then
...
procedure TSimpleCodec.SetAsymGenProgressEvent(Value: TGenerateAsymetricKeyPairProgress);
begin
...
begin
  ...
  FisUserAborted := False;
  FisSalting := False;
  if (not isAsymetric) or (not(Supports(FParameterizedStreamCipher, IAsymetric_Engine, Asymetric_Engine))) then
    raise Exception.Create(ES_AsymNotInitByStr);
  FreeAndNil(FKey);
  Asymetric_Engine.GenerateAsymetricKeyPair(FAsymetricKeySizeInBits, FSender, FAsymGenProgressEvent, KeyPair, FisUserAborted);
  if FisUserAborted then
    FMode := cmUnitialized
  else
    begin
...
procedure TSimpleCodec.Sign(Document, Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress;
  SigningKeys_PrivatePart: TObject; // Must be uTPLb_Asymetric.TAsymtricKeyPart
  var wasAborted: boolean);
var
...
begin
  ...
  Engine.Sign(Document, Signature, SigningKeys_PrivatePart as TAsymtricKeyPart, ProgressSender, ProgressEvent, wasAborted)
end;

function TSimpleCodec.VerifySignature(Document, Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress;
  SigningKeys_PublicPart: TObject; // Must be uTPLb_Asymetric.TAsymtricKeyPart
  var wasAborted: boolean): boolean;
var
  Engine: IAsymetric_Engine;
...
begin
  ...
  // Typically, this is an instance of TAsymetric_Engine.
  // EstimatedWorkLoad := Document.Size;
  result := Engine.VerifySignature(Document, Signature, SigningKeys_PublicPart as TAsymtricKeyPart, ProgressSender, ProgressEvent,
    wasAborted)
end;
...
procedure TCodec.DecryptFile(const Plaintext_FileName, CipherText_FileName: string);
begin
  InterfacesAreCached := True;
  BeginEncDec;
...
procedure TCodec.EncryptFile(const Plaintext_FileName, CipherText_FileName: string);
begin
  InterfacesAreCached := True;
  BeginEncDec;
...
procedure TCodec.GenerateAsymetricKeyPairProgress_Event(Sender: TObject; CountPrimalityTests: integer; var doAbort: boolean);
var
...
begin
  if FCodec.GetAdvancedOptions2 = Value then
    exit;
  InterfacesAreCached := False;
