...
type
  ...
  TStreamToBlock_Adapter = class(TInterfacedObject, IStreamCipher, ICryptoGraphicAlgorithm, IControlObject, IStreamCipherEx2)
  private
...
type
  ...
  public
    constructor Start_EncDec(Key1: TSymetricKey; EmissionStream1: TStream; const BlockCipher1: IBlockCipher;
  ...
  public
    constructor Start_Encrypt(Key1: TSymetricKey; CipherText1: TStream; const BlockCipher1: IBlockCipher;
  ...
  public
    ...
    constructor Start_Encrypt_WithIV(Key1: TSymetricKey; CipherText1: TStream; const BlockCipher1: IBlockCipher;
  ...
  public
    constructor Start_Decrypt(Key1: TSymetricKey; PlainText1: TStream; const BlockCipher1: IBlockCipher;
      const Chaining1: IBlockChainingModel);
  ...
  public
    ...
    constructor Start_Decrypt_WithIV(Key1: TSymetricKey; PlainText1: TStream; const BlockCipher1: IBlockCipher;
      const Chaining1: IBlockChainingModel; IV1: TStream);
...
function TStreamToBlock_Adapter.Parameterize(const Params: IInterface): IStreamCipher;
var
...
begin
  result := nil;
  if assigned(FBlockCipher) and assigned(FChaining) then
    exit;
  FAdvancedOptions := [];
  ...
  if not(assigned(BlockCipher) or assigned(Chaining)) then
    exit;
  Newbie := TStreamToBlock_Adapter.Create;
  Newbie.FBlockCipher := BlockCipher;
  Newbie.FChaining := Chaining;
  result := Newbie;
  if (not assigned(BlockCipherSelectorEx2)) or (not BlockCipherSelectorEx2.hasOnSetIVHandler(Newbie.FOnSetIV)) then
    Newbie.FOnSetIV := nil
end;
...
function TStreamToBlock_Adapter.Start_Decrypt(Key: TSymetricKey; PlainText: TStream): IStreamDecryptor;
begin
  if optOpenSSL_CompatibilityMode in FAdvancedOptions then
    begin
      ...
      result := TOpenSSLCompatDecryptor.Start_Decrypt(Key, PlainText, FBlockCipher, FChaining)
    end

  else if cfNoNounce in FChaining.ChainingFeatures then
    result := TNoNonceDecryptor.Start_Decrypt(Key, PlainText, FBlockCipher, FChaining)
  else
    result := TNoncibleDecryptor.Start_Decrypt(Key, PlainText, FBlockCipher, FChaining)
end;

function TStreamToBlock_Adapter.Start_Encrypt(Key: TSymetricKey; CipherText: TStream): IStreamEncryptor;
begin
  if optOpenSSL_CompatibilityMode in FAdvancedOptions then
    ...
    begin
      if not(cfNoNounce in FChaining.ChainingFeatures) then
        raise Exception.Create(ES_OpenSSLCompat_RequiresIV);
      result := TOpenSSLCompatEncryptor.Start_Encrypt(Key, CipherText, FBlockCipher, FChaining)
    end

  else if cfNoNounce in FChaining.ChainingFeatures then
    result := TNoNonceEncryptor.Start_Encrypt(Key, CipherText, FBlockCipher, FChaining)
  else
    result := TNoncibleEncryptor.Start_Encrypt(Key, CipherText, FBlockCipher, FChaining, FOnSetIV)
end;

function TStreamToBlock_Adapter.Start_Decrypt(Key: TSymetricKey; PlainText, IV: TStream): IStreamDecryptor;
var
  sName: string;
begin
  if optOpenSSL_CompatibilityMode in FAdvancedOptions then
    result := TOpenSSLCompatDecryptor.Start_Decrypt_WithIV(Key, PlainText, FBlockCipher, FChaining, IV)
  else
    begin
      if assigned(FBlockCipher) then
...
function TStreamToBlock_Adapter.Start_Encrypt(Key: TSymetricKey; CipherText, IV: TStream): IStreamEncryptor;
var
  sName: string;
begin
  if optOpenSSL_CompatibilityMode in FAdvancedOptions then
    result := TOpenSSLCompatEncryptor.Start_Encrypt_WithIV(Key, CipherText, FBlockCipher, FChaining, IV)
  else
    begin
      if assigned(FBlockCipher) then
        sName := FBlockCipher.DisplayName
...
constructor TNoNonceEncryptor.Start_Encrypt(Key1: TSymetricKey; CipherText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel);
begin
...
constructor TNoncibleEncryptor.Start_Encrypt(Key1: TSymetricKey; CipherText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel; OnSetIV1: TSetMemStreamProc);
begin
  inherited Start_EncDec(Key1, CipherText1, BlockCipher1, Chaining1, True);
  FKey := Key1;
    ...
    begin
      if FisShortMessage and (not(cfKeyStream in FChaining.ChainingFeatures)) then
        begin
          ...
          Fis8bitMode := (cf8bit in Actual.ChainingFeatures) and (cfKeyStream in Actual.ChainingFeatures)
        end
      else
        Actual := FChaining;
      FisAutoXOR := ([cfKeyStream, cfAutoXOR] * Actual.ChainingFeatures) = [cfKeyStream, cfAutoXOR];
      if not assigned(FChainState) then
      ...
      if FLenBuffer = 0 then
        ...
        begin // Ciphertext stealing
          ...
          try
            ...
            Move(MemStrmOffset(FEmitBuffer, FLenBuffer)^, MemStrmOffset(FBuffer, FLenBuffer)^, FBlockLen - FLenBuffer);
            // << The steal!
            FBuffer.Position := 0;
...
begin
  BurnMemoryStream(F2ndBuffer);
  FreeAndNil(F2ndBuffer);
...
constructor TNoNonceDecryptor.Start_Decrypt(Key1: TSymetricKey; PlainText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel);
begin
...
begin
  if FDataCount = 0 then
    ...
    begin
      Assert(FisSecondBufferFull and (FLenBuffer = 0), AS_BlockPaddingCorrupt);
...
constructor TNoncibleDecryptor.Start_Decrypt(Key1: TSymetricKey; PlainText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel);
begin
  inherited Start_EncDec(Key1, PlainText1, BlockCipher1, Chaining1, False);
  FKey := Key1;
...
constructor TEncryptorDecryptor.Start_EncDec(Key1: TSymetricKey; EmissionStream1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel; isEncrypting1: boolean);
begin
  ...
  FChainState := nil;
  FDataCount := 0;
  FisAutoXOR := ([cfKeyStream, cfAutoXOR] * FChaining.ChainingFeatures) = [cfKeyStream, cfAutoXOR];
  Fis8bitMode := (cf8bit in FChaining.ChainingFeatures) and (cfKeyStream in FChaining.ChainingFeatures) and
...
procedure TEncryptorDecryptor.Chained_Decrypt_Block(PlainText, CipherText: TMemoryStream);
begin
  PlainText.Position := 0;
  CipherText.Position := 0;
...
procedure TEncryptorDecryptor.Chained_Encrypt_Block(PlainText, CipherText: TMemoryStream);
begin
  PlainText.Position := 0;
  CipherText.Position := 0;
...
procedure TEncryptorDecryptor.Emit(SourceBuf: TMemoryStream; EmitLen: integer = -1);
var
...
constructor TOpenSSLCompatEncryptor.Start_Encrypt(Key1: TSymetricKey; CipherText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel);
begin
  // Valid only for ECB. Other chain modes for OpenSSL compatibility
  // require an explicit user-supplied IV.
...
constructor TOpenSSLCompatEncryptor.Start_Encrypt_WithIV(Key1: TSymetricKey; CipherText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel; IV1: TStream);
begin
  inherited Start_EncDec(Key1, CipherText1, BlockCipher1, Chaining1, True);
  FKey := Key1;
...
constructor TOpenSSLCompatDecryptor.Start_Decrypt(Key1: TSymetricKey; PlainText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel);
begin
  // TODO
...
constructor TOpenSSLCompatDecryptor.Start_Decrypt_WithIV(Key1: TSymetricKey; PlainText1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel; IV1: TStream);
begin
  // TODO
...
type
  TCSharpStreamToBlock_Adapter = class(TInterfacedObject, IStreamCipher, ICryptoGraphicAlgorithm, IControlObject)
  private
    FBlockCipher: IBlockCipher;
    FChaining: IBlockChainingModel;
  ...
  public
    constructor Start_Decrypt(Key1: TSymetricKey; EmissionStream1: TStream; const BlockCipher1: IBlockCipher;
      const Chaining1: IBlockChainingModel);
...
function TCSharpStreamToBlock_Adapter.Parameterize(const Params: IInterface): IStreamCipher;
var
  BlockCipherSelector: IBlockCipherSelector;
  // BlockCipherSelectorEx2: IBlockCipherSelectorEx2;
  BlockCipher: IBlockCipher;
...
function TCSharpStreamToBlock_Adapter.Start_Decrypt(Key: TSymetricKey; PlainText: TStream): IStreamDecryptor;
begin
  if cfNoNounce in FChaining.ChainingFeatures then
    NotSupported
  else
    result := TCSharpDecryptor.Start_Decrypt(Key, PlainText, FBlockCipher, FChaining)
end;

function TCSharpStreamToBlock_Adapter.Start_Encrypt(Key: TSymetricKey; CipherText: TStream): IStreamEncryptor;
begin
  NotSupported
end;
...
constructor TCSharpDecryptor.Start_Decrypt(Key1: TSymetricKey; EmissionStream1: TStream; const BlockCipher1: IBlockCipher;
  const Chaining1: IBlockChainingModel);
begin
  FEmissionStream := EmissionStream1;
  FBlockCipher := BlockCipher1;
...
procedure TCSharpDecryptor.Chained_Decrypt_Block(PlainText, CipherText: TMemoryStream);
begin
  PlainText.Position := 0;
  CipherText.Position := 0;
  FChainState.Decrypt_Block(PlainText, CipherText)
...
procedure TCSharpDecryptor.Emit(SourceBuf: TMemoryStream; EmitLen: integer = -1);
var
  PlaintextByte, CiphertextByte: byte;
  P, Q: PByte;
  j: integer;
