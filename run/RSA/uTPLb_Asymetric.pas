...
type
  ...
  public
    ...
    function VerifySignature(Document: TStream; // FCipherText is the signature to be verified.
      ProgressSender: TObject; ProgressEvent: TOnEncDecProgress; var wasAborted: boolean): boolean; virtual; abstract;
  end;
  ...
  public
    ...
    procedure Sign( // FPlaintext is the document to be signed.
      Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress; var wasAborted: boolean); virtual; abstract;
  end;
  ...
  IAsymetric_Engine = interface(IStreamCipher)
    ...
    procedure Sign(Document, Signature: TStream; PrivatePart: TAsymtricKeyPart; ProgressSender: TObject;
      ProgressEvent: TOnEncDecProgress; var wasAborted: boolean);

    function VerifySignature(Document, Signature: TStream; PublicPart: TAsymtricKeyPart; ProgressSender: TObject;
  ...
  TAsymetric_Engine = class(TInterfacedObject, IStreamCipher, ICryptoGraphicAlgorithm, IAsymetric_Engine)
  protected
  ...
  public
    procedure GenerateAsymetricKeyPair(KeySizeInBits: cardinal; ProgressSender: TObject;
      ProgressEvent: TGenerateAsymetricKeyPairProgress; var KeyPair: TAsymetricKeyPair; var wasAborted: boolean); virtual; abstract;

    procedure Sign(Document, Signature: TStream; PrivatePart: TAsymtricKeyPart; ProgressSender: TObject;
      ProgressEvent: TOnEncDecProgress; var wasAborted: boolean); virtual;

    function VerifySignature(Document, Signature: TStream; PublicPart: TAsymtricKeyPart; ProgressSender: TObject;
      ProgressEvent: TOnEncDecProgress; var wasAborted: boolean): boolean; virtual;

    function CreateFromStream(Store: TStream; Parts: TKeyStoragePartSet): TAsymetricKeyPair; virtual; abstract;
  end;
...
function TAsymetricKeyPair.Can_StoreToStream(Parts: TKeyStoragePartSet): boolean;
begin
...
procedure TAsymetricKeyPair.StoreToStream(Store: TStream; Parts: TKeyStoragePartSet);
var
  hasPart: boolean;
begin
...
(*
procedure TAsymetricKeyPair_DescendantClass.LoadFromStream(Store: TStream; Parts: TKeyStoragePartSet);
var
  hasPart: boolean;
begin
  if partPublic in Parts then
...
begin
  result := [afOpenSourceSoftware, afDoesNotNeedSalt, // Only applicable to IStreamCiphers which are true
...
function TAsymetric_Engine.Parameterize(const Params: IInterface): IStreamCipher;
begin
  result := nil
end;
...
procedure TAsymetric_Engine.Sign(Document, Signature: TStream; PrivatePart: TAsymtricKeyPart; ProgressSender: TObject;
  ProgressEvent: TOnEncDecProgress; var wasAborted: boolean);
var
...
function TAsymetric_Engine.Start_Decrypt(Key: TSymetricKey; Plaintext: TStream): IStreamDecryptor;
begin
  if (Key is TAsymetricKeyPair) and assigned(TAsymetricKeyPair(Key).FPrivatePart) then
    result := DecClass.Start_Decrypt(TAsymetricKeyPair(Key).FPrivatePart, Plaintext)
  else
    result := nil
end;

function TAsymetric_Engine.Start_Encrypt(Key: TSymetricKey; Ciphertext: TStream): IStreamEncryptor;
begin
  if (Key is TAsymetricKeyPair) and assigned(TAsymetricKeyPair(Key).FPublicPart) then
    result := EncClass.Start_Encrypt(TAsymetricKeyPair(Key).FPublicPart, Ciphertext)
  else
    result := nil
end;

function TAsymetric_Engine.VerifySignature(Document, Signature: TStream; PublicPart: TAsymtricKeyPart; ProgressSender: TObject;
  ProgressEvent: TOnEncDecProgress; var wasAborted: boolean): boolean;
var
...
begin
  ...
  try
    ...
    wasAborted := False;
    Document.Position := 0;
    result := EncObj.VerifySignature(Document, ProgressSender, ProgressEvent, wasAborted)
  finally
    EncObj._Release;
    if not isCounted then
      EncObj.Free
...
constructor TAsymetricEncryptor.Start_Encrypt(PublicKey1: TAsymtricKeyPart; CipherText1: TStream);
begin
  inherited Create;
...
begin
  ...
  if Ciphertext is TMemoryStream then
    begin
      FSymetricCodec.DecryptMemory(MemStrmOffset(TMemoryStream(Ciphertext), Ciphertext.Position)^, Sz);
...
constructor TAsymetricDecryptor.Start_Decrypt(PrivateKey1: TAsymtricKeyPart; PlainText1: TStream);
begin
  inherited Create;
