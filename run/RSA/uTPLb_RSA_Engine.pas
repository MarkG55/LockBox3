...
type
  ...
  public
    procedure GenerateAsymetricKeyPair(KeySizeInBits: cardinal; ProgressSender: TObject;
      ProgressEvent: TGenerateAsymetricKeyPairProgress; var KeyPair: TAsymetricKeyPair; var wasAborted: boolean); override;

    function CreateFromStream(Store: TStream; Parts: TKeyStoragePartSet): TAsymetricKeyPair; override;
  end;
...
type
  ...
  protected
    ...
    procedure SenseVersion(doReadPastHeader: boolean; Store: TStream; var Version: integer; var AvailableParts: RSAKeyStorePartSet);
  ...
  protected
    ...
    function StoreHugeCardinal(Number: THugeCardinal; StoreStream: TStream): boolean; virtual;
    function LoadHugeCardinal_IfNotAlready(StoreStream: TStream; var Number: IHugeCardinalWrap): boolean; virtual;
  ...
  public
    ...
    function VerifySignature(Document: TStream; // FCipherText is the signature to be verified.
      ProgressSender: TObject; ProgressEvent: TOnEncDecProgress; var wasAborted: boolean): boolean; override;
  end;
  ...
  public
    ...
    procedure Sign( // FPlaintext is the document to be signed.
      Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress; var wasAborted: boolean); override;
  end;
...
type
  ...
  private
    ...
    procedure Progress_Event(Sender: TObject; BitsProcessed, TotalBits: int64; var doAbort: boolean);
    procedure PrimalityTest_Event(CountPrimalityTests: integer);

  public
    constructor Create(Client_ProgressEvent1: TGenerateAsymetricKeyPairProgress; const Pool1: IMemoryStreamPool);
    destructor Destroy; override;
  end;
...
procedure TRSA_Engine.GenerateAsymetricKeyPair(KeySizeInBits: cardinal; ProgressSender: TObject;
  ProgressEvent: TGenerateAsymetricKeyPairProgress; var KeyPair: TAsymetricKeyPair; var wasAborted: boolean);
var
...
begin
  ...
  try
    uTPLb_HugeCardinalUtils.Compute_RSA_Fundamentals_2Factors(KeySizeInBits, StandardExponent, N, e, d, Totient, p, q, dp, dq, qinv,
      Helper.Progress_Event, Helper.PrimalityTest_Event, GeneratePrimePassCount, RSAKeyPair.FPool, Helper.FNumbersTested, wasAborted);
...
function TRSA_Engine.CreateFromStream(Store: TStream; Parts: TKeyStoragePartSet): TAsymetricKeyPair;
begin
...
procedure TRSAKeyPart.SenseVersion(doReadPastHeader: boolean; Store: TStream; var Version: integer;
...
begin
  ...
  if Version >= 1 then
    begin
      ...
      if ((Parts * [PartD, PartCRT]) <> []) and (PartCRT in AvailableParts) then
...
begin
  ...
  result := Generate_RSA_SymetricKey(Key.F_RSA_n.Value, Key.F_RSA_e.Value, FCipherText, FSymetricCodec.BlockCipher)
end;
...
function TRSA_Encryptor.VerifySignature(Document: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress;
...
begin
  Key := FPublicKey as TRSA_PublicKeyPart;
  wasAborted := False;
  result := assigned(Key) and assigned(Key.F_RSA_n) and (not Key.F_RSA_n.IsZero) and assigned(Key.F_RSA_e) and
    (not Key.F_RSA_e.IsZero);
  if result then
    begin
      Monitor := TMonitor.Create(ProgressEvent, ProgressSender);
      try
        OpRes := RSASSA_PSS_VERIFY(Key.F_RSA_n.Value, Key.F_RSA_e.Value, Document, FCipherText, Monitor.OnProgress)
      finally
        Monitor.Free;
      end;
      result := OpRes = opPass;
...
begin
  ...
  result := Extract_RSA_SymetricKey(UnWrap(Key.F_RSA_d), UnWrap(Key.F_RSA_n), UnWrap(Key.F_RSA_p), UnWrap(Key.F_RSA_q),
    UnWrap(Key.F_RSA_dp), UnWrap(Key.F_RSA_dq), UnWrap(Key.F_RSA_qinv), Ciphertext, FSymetricCodec.BlockCipher)
end;

procedure TRSA_Decryptor.Sign(Signature: TStream; ProgressSender: TObject; ProgressEvent: TOnEncDecProgress; var wasAborted: boolean);
var
  Succeeded: boolean;
  Key: TRSA_PrivateKeyPart;
  OpRes: TLongOpResult;
...
begin
  wasAborted := False;
  Key := FPrivateKey as TRSA_PrivateKeyPart;
  Succeeded := assigned(Key) and assigned(Key.F_RSA_d) and (not Key.F_RSA_d.IsZero) and assigned(Key.F_RSA_n) and
    (not Key.F_RSA_n.IsZero);
  if Succeeded then
    begin
      Monitor := TMonitor.Create(ProgressEvent, ProgressSender);
      try
        OpRes := uTPLb_RSA_Primitives.RSASSA_PSS_SIGN(UnWrap(Key.F_RSA_d), UnWrap(Key.F_RSA_n), FPlainText, Signature,
          Monitor.OnProgress, UnWrap(Key.F_RSA_p), UnWrap(Key.F_RSA_q), UnWrap(Key.F_RSA_dp), UnWrap(Key.F_RSA_dq),
          UnWrap(Key.F_RSA_qinv))
      finally
        Monitor.Free
      end;
...
function StoreHugeCardinal_Primitive(Number: THugeCardinal; StoreStream: TStream): boolean;
...
begin
  ...
  if result then
    ...
    L := 0;
  if not assigned(StoreStream) then
    exit;
...
function LoadHugeCardinal_Primitive(StoreStream: TStream; const Pool1: IMemoryStreamPool): THugeCardinal;
...
begin
  ...
  try
    ...
    result := THugeCardinal.CreateFromStreamIn(L * 8, LittleEndien, ValueStream, Pool1)
  finally
    ValueStream.Free
  end
end;

function TRSAKeyPair.StoreHugeCardinal(Number: THugeCardinal; StoreStream: TStream): boolean;
// virtual method.
begin
...
function TRSAKeyPair.LoadHugeCardinal_IfNotAlready(StoreStream: TStream; var Number: IHugeCardinalWrap): boolean;
// virtual method.
var
  L: cardinal;
  ValueStream: TMemoryStream;
begin
  ...
  if not result then
    exit; // Only load if we are not already loaded.
  ...
  try
    ...
    if L > 0 then
      StoreStream.ReadBuffer(ValueStream.Memory^, L);
    ValueStream.Position := 0;
    Number := NewWrap(THugeCardinal.CreateFromStreamIn(L * 8, LittleEndien, ValueStream, FPool))
  finally
    ValueStream.Free
  end;
...
procedure TRSA_PublicKeyPart.MarkPartsToStoreLoad(var Parts: RSAKeyStorePartSet);
begin
  Include(Parts, PartE)
end;
...
procedure TRSA_PrivateKeyPart.MarkPartsToStoreLoad(var Parts: RSAKeyStorePartSet);
begin
  Include(Parts, PartD);
...
begin
  if not((FPublicPart is TRSA_PublicKeyPart) and (FPrivatePart is TRSA_PrivateKeyPart) and
    (TRSA_PublicKeyPart(FPublicPart).FOwner = self) and (TRSA_PublicKeyPart(FPublicPart).F_RSA_n = F_RSA_n) and
    (TRSA_PublicKeyPart(FPublicPart).F_RSA_e = F_RSA_e) and (TRSA_PrivateKeyPart(FPrivatePart).FOwner = self) and
    (TRSA_PrivateKeyPart(FPrivatePart).F_RSA_n = F_RSA_n) and (TRSA_PrivateKeyPart(FPrivatePart).F_RSA_d = F_RSA_d) and
    (TRSA_PrivateKeyPart(FPrivatePart).F_RSA_p = F_RSA_p) and (TRSA_PrivateKeyPart(FPrivatePart).F_RSA_q = F_RSA_q) and
    (TRSA_PrivateKeyPart(FPrivatePart).F_RSA_dp = F_RSA_dp) and (TRSA_PrivateKeyPart(FPrivatePart).F_RSA_dq = F_RSA_dq) and
...
constructor TRSA_Gen_Key_Helper.Create(Client_ProgressEvent1: TGenerateAsymetricKeyPairProgress; const Pool1: IMemoryStreamPool);
begin
  FPool := Pool1;
...
procedure TRSA_Gen_Key_Helper.Progress_Event(Sender: TObject; BitsProcessed, TotalBits: int64; var doAbort: boolean);
begin
...
function TMonitor.OnProgress(Sender: TObject; CountBytesProcessed: int64): boolean;
begin
