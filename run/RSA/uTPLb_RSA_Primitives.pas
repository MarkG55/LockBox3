...
function I2OSP(x: THugeCardinal; xLen: integer; XStream: TStream; const Pool: IMemoryStreamPool): boolean;
...
function OS2IP(XStream: TStream; xLen: integer; var x: THugeCardinal; const Pool: IMemoryStreamPool; MaxBits: integer): boolean;
...
function RSAES_OAEP_DECRYPT(d, n: THugeCardinal; C, M: TStream; p, q, dp, dq, qinv: THugeCardinal): boolean;
...
function EMSA_PSS_ENCODE(M: TStream; emBits, sLen: integer; EM: TStream; CheckAbortFunc: TOnHashProgress): TLongOpResult;
...
function RSASSA_PSS_SIGN(d, n: THugeCardinal; M, S: TStream; CheckAbortFunc: TOnHashProgress; p, q, dp, dq, qinv: THugeCardinal)
  : TLongOpResult;
...
function EMSA_PSS_VERIFY(M: TStream; emBits, sLen: integer; EM: TStream; CheckAbortFunc: TOnHashProgress): TLongOpResult;
// EMSA-PSS-VERIFY
// It is the inverse of  EMSA-PSS-ENCODE

function RSASSA_PSS_VERIFY(n, e: THugeCardinal; M, S: TStream; CheckAbortFunc: TOnHashProgress): TLongOpResult;
// RSASSA-PSS-VERIFY
// Inputs:
// n         The signer's RSA public key modulus.
...
function Generate_RSA_SymetricKey(n, e: THugeCardinal; CipherStream: TStream; const SymetricCipher: IBlockCipher): TSymetricKey;

function Extract_RSA_SymetricKey(d, n, p, q, dp, dq, qinv: THugeCardinal; CipherStream: TStream; const SymetricCipher: IBlockCipher)
  : TSymetricKey;
...
array [0 .. V3_0_0_BaseIdx { Insert RSA_Primitives_AlgorithmVersion here } ] of record TPLB3_Version_Low, TPLB3_Version_High: string;
end
= (
...
function OS2IP(XStream: TStream; xLen: integer; var x: THugeCardinal; const Pool: IMemoryStreamPool; MaxBits: integer): boolean;
begin
  if (RSA_Primitives_AlgorithmVersion <> V3_0_0_BaseIdx) and ((XStream.Size - XStream.Position) <> xLen) then
...
{$ELSE}
    if (not OS2IP(EM, k, m1, n.FPool, n.BitLength)) or (m1.Compare(n) in [rGreaterThan, rEqualTo]) then
      raise Exception.Create('RSAES_OAEP_ENCRYPT ' + ES_InternalError_Suffix);
{$ENDIF}
    // Step 3.b
    // c = m ** e mod n; // RSAEP
...
{$ENDIF}
    ...
    result := CompareMemoryStreams(reconHash, HashOfNil) and (Y = 1) and (bytesRead = 1) and (mLen > 0);
    if not result then
      exit;
...
begin
  ...
  try
    ...
    try
      ...
      H.Size := hLen1;
      Ok := Ok and (EM.Read(H.Memory^, hLen1) = hLen1) and (EM.Read(aByte, 1) = 1) and (aByte = $BC); // checks $BC byte.
      if not Ok then
        exit;
...
function TDocumentProgressMonitor.OnHashProgress(Sender: TObject; CountBytesProcessed: int64): boolean;
begin
